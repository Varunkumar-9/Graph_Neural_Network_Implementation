...
class GNNStack(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args, task):
...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, reducer='mean', 
        ...
        self.agg_lin = nn.Linear(self.in_channels, self.out_channels) 
        ############################################################################
    ...
    def forward(self, x, edge_index):
        ...
        out = None # TODO
        out = self.lin(x) 
        self.propagate(edge_index, size=(num_nodes, num_nodes),x=out) 
        out = self.agg_lin(torch.cat([out,x], dim=-1))
        ############################################################################
        return out
...
class GAT(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, num_heads=1, concat=True,
        ...
        #self.lin = None # TODO
        self.lin=nn.Linear(self.in_channels,self.out_channels*self.heads,bias=False)
        ############################################################################
    ...
    def forward(self, x, edge_index, size=None):
        ...
        x = None # TODO
        #x=self.att*(self.lin(x).reshape(-1,self.heads,self.out_channels))
